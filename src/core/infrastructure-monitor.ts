...
// DNS-over-HTTPS resolver for looking up DNS records
interface DNSResolver {
  resolve(domain: string, type: string): Promise<string[]>;
}
...
export class InfrastructureMonitor {
  ...
  private domainCache: Map<string, DomainRecord> = new Map();
  private ipCache: Map<string, IPRecord> = new Map();
  private certCache: Map<string, CertificateInfo> = new Map();

  /**
   * Add a domain to monitoring
   */
  async addDomain(domain: string, organizationId: string): Promise<MonitoredAsset> {
    const id = uuidv4();
   ...
   */
  async addIP(ip: string, organizationId: string): Promise<MonitoredAsset> {
    const id = uuidv4();
    const asset: MonitoredAsset = {
      id,
      type: 'ip',
   ...
   */
  async addCertificate(domain: string, organizationId: string): Promise<MonitoredAsset> {
    const id = uuidv4();
    const asset: MonitoredAsset = {
      id,
      type: 'certificate',
   ...
   */
  async lookupDomain(domain: string): Promise<DomainRecord> {
   ...
   */
  async lookupIP(ip: string): Promise<IPRecord> {
    const version = net.isIP(ip) as 4 | 6;
   ...
   */
  async lookupCertificate(domain: string): Promise<CertificateInfo> {
   ...
   */
  checkCertificateExpiry(cert: CertificateInfo): { isExpiring: boolean; daysLeft: number; severity: 'critical' | 'high' | 'medium' | 'low' } {
   ...
   */
  checkDomainSecurity(domain: string): { hasSPF: boolean; hasDMARC: boolean; hasDKIM: boolean; issues: string[] } {
   ...
   */
  private resolveDNS(domain: string, type: string): Promise<string[]> {
   ...
   */
  private reverseLookup(ip: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
   ...
   */
  async createSampleInfrastructure(orgId: string): Promise<void> {
    // Sample domains
