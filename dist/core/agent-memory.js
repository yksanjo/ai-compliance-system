...
export class AgentMemory {
     ...
     */
    storeKnowledge(content, tags = [], source = 'user') {
        const embedding = this.generateEmbedding(content);
        const entry = {
            id: uuidv4(),
            type: 'knowledge',
     ...
     */
    addConversation(role, content, context = { policies: [], violations: [] }) {
        const entry = {
            id: uuidv4(),
            role,
            content,
     ...
     */
    getContext(query, maxEntries = 10) {
    ...
    createSampleMemories() {
        // Sample policies
        this.storeKnowledge('SOC2 requires annual penetration testing by qualified third party. Testing scope must include all in-scope systems and network infrastructure.', ['SOC2', 'penetration-testing', 'annual'], 'compliance-knowledge');
        this.storeKnowledge('ISO 27001 requires documented incident response procedures. All security incidents must be reported within 24 hours.', ['ISO27001', 'incident-response', '24h'], 'compliance-knowledge');
        // Sample violations
        this.storeKnowledge('Certificate for api.example.com expires in 15 days - renewal required', ['certificate', 'expiry', 'warning'], 'infrastructure-alert');
        this.storeKnowledge('Domain example.com missing SPF record - email spoofing risk', ['domain', 'spf', 'security'], 'infrastructure-alert');
    }
}
