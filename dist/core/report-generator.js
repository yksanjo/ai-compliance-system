...
export class ReportGenerator {
    ...
    generateReport(options) {
        ...
        const frameworkViolations = violations.filter(v => frameworkPolicies.some(p => p.id === v.policyId));
     ...
     */
    generateSummary(policies, violations) {
        ...
        const nonCompliantControls = violations.filter(v => v.status !== 'resolved' && v.status !== 'false_positive').length;
        ...
        const riskScore = Math.min(100, (criticalFindings * 10) + (highFindings * 5) + (mediumFindings * 2));
        // Determine trend
        let trend = 'stable';
     ...
     */
    generateControlStatus(policies, violations) {
        const controls = [];
        for (const policy of policies) {
            for (const control of policy.controls) {
                const controlViolations = violations.filter(v => v.controlId === control.id && v.status !== 'resolved');
     ...
     */
    generateEvidence(violations, policies) {
        const evidence = [];
